...
// Otherwise, request the permission
    ...
    private fun openImageCapture() {
        // 1. Buat variabel LOKAL non-nullable
        val newPhotoInfo = providerFileManager.generatePhotoUri(System.currentTimeMillis())

        // 2. Set properti CLASS untuk digunakan oleh callback nanti
        photoInfo = newPhotoInfo

        // 3. Panggil launch() menggunakan variabel LOKAL (yang dijamin non-null)
        takePictureLauncher.launch(newPhotoInfo.uri)
    }
    // Open the camera to capture a video
    private fun openVideoCapture() {
        // 1. Buat variabel LOKAL non-nullable
        val newVideoInfo = providerFileManager.generateVideoUri(System.currentTimeMillis())

        // 2. Set properti CLASS untuk digunakan oleh callback nanti
        videoInfo = newVideoInfo

        // 3. Panggil launch() menggunakan variabel LOKAL (yang dijamin non-null)
        takeVideoLauncher.launch(newVideoInfo.uri)
    }
    // Check the storage permission
